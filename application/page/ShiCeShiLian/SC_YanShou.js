    ...
    static navigationOptions = ({navigation}) => {
        return {
            ...
            headerRight: <Freedomen.Region 
                event={params => {
                    if (thisParams != null) {
                        Freedomen.global.api.post('api/measuredProblem/measuredProblemCheckAgain', {
                            measuredId: navigation.state.params.id,
                            score: thisParams.score || 0,
                            state: thisParams.state ? 1 : 0
                        }).then(res => {
                            Freedomen.global.fn()
                            navigation.goBack()
                        })
